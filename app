...
const Btn = ({ ghost, danger, className = "", ...p }) => (
  <button
    className={[
      "px-3 py-1.5 rounded-lg text-sm border shadow-sm",
      danger
...
const Input = ({ className = "", ...p }) => (
  <input
    className={[
      "border rounded-lg px-3 py-1.5 w-full bg-gray-900 text-white border-gray-700",
      className,
    ].join(" ")}
    {...p}
  />
...
// --- default state (lean)
const defaultTimes = ["08:00", "11:00", "14:00", "17:00", "20:00"]; // 5×/day
const defaultData = {
  version: 2,
